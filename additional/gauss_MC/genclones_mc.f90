...
integer::i,j,info,nclones,dimen
integer,dimension(:),allocatable::iwork
real*8,dimension(6,6)::covceke,covke,covceket,covce,atri
...
character::dumc,elec
 ...
 !calculate new parameters following bancelin's thesis P=A.eps+P0 where eps is a vector of gaussian random variables
 !and A is the upper tridiagonal of the covariance matrix. Here A=amat
 amat=covke
 
 
 call DPOTRF( 'U', dimen, amat, dimen, info )
 
 if (info.ne.0) then
  write(*,*)'Error in Lapack subroutine DPOTRF'
  STOP
 end if
